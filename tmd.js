...
(async function () {
  ...
  const region = (typeof TMD_REGION !== 'undefined') ? TMD_REGION : null;

  /* ── 2. Render instructors ── */
  const instructorTarget = document.getElementById('tmd-instructors');
  if (instructorTarget && (suburb || region)) {
    // Region pages show all instructors; suburb pages filter by suburb name
